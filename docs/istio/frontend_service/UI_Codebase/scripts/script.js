$(document).ready(function(){
	...
	function requestNetworkNodes(requestParam){
		$.ajax({
			...
			success: function (data) {
				$("#alertMessage").removeClass('hidden').addClass('alert-success');
				$("#alertMessage .content").html(data.reserved_ip_success.join()+' IP addresses has been reserved');
			},
			error: function (jqXhr, textStatus, errorMessage) {
				console.log('Error' + errorMessage);
			}
	...
	function initiateDashboard(){
		let selectedList = [],nodeListData = [];
		makeNetworkCall();	
		generateNetworkList = (dataList) => {
			var dataString = "";
			for(let i=0;i<dataList.length;i++){
				dataString = dataString + "<div class='panel panel-default'><div class='panel-heading' role='tab' id='"+'heading'+i+"'><h4 class='panel-title'><a role='button' data-toggle='collapse' data-parent='#accordion' href='"+'#collapse'+i+"' aria-expanded='false' aria-controls='"+'collapse'+i+"' class='link'>"+'Network Name : '+dataList[i].network+"</a></h4></div><div id='"+'collapse'+i+"' class='panel-collapse collapse' role='tabpanel' aria-labelledby='"+'heading'+i+"'><div class='panel-body'></div></div></div>"
		...
		generateNodeList = (data,e) =>{
			...
			for(let i=0;i<data.length;i++){
				...
				if(data[i].status == 'USED'){
				  nodeList = nodeList + "<input type='checkbox' id='"+'networkIP'+i+"' data-network-id='"+data[i].ip_address+"' class='inputBox' disabled>"
				}else{
				  nodeList = nodeList + "<input type='checkbox' id='"+'networkIP'+i+"' data-network-id='"+data[i].ip_address+"' class='inputBox'>"
				}		
			}
			nodeList = nodeList + "<section class='btn-wrapper'><button class='requestNetwork'>Request</button><button  class='releaseNetwork'>Release</button></section>"
			$(e.parentElement.parentNode.nextElementSibling.children[0]).append(nodeList);
		}
